...
-(void)test
{
    ...
    [globalHelper search:[LKTest class] where:nil orderBy:nil offset:0 count:100 callback:^(NSMutableArray *array) {
		...
		NSString *filePath = [[NSBundle mainBundle] pathForResource:@"industry" ofType:@"xml"];
		NSData *xmlData = [[NSData alloc] initWithContentsOfFile:filePath];
		//

		LKDBHelper* globalHelper = [LKTTMSG getUsingLKDBHelper];
		array =  [globalHelper search:[LKTTMSG class] where:nil orderBy:nil offset:0 count:100];
		//
		NSLog(@"result_ttmsg:(%@)",array);
    }];
}
@end
